...
export default function QuotesTab({ onUnauthorised }) {
  ...
  return (
    <div>
      <MailStatus onUnauthorised={onUnauthorised} />

      {error ? (
        <p className="text-sm text-destructive">{error}</p>
      ) : !data ? (
        <p className="text-sm font-light text-ink/55">Loading…</p>
      ) : !data.length ? (
        <p className="text-sm font-light text-ink/55">No quotations yet.</p>
      ) : (
